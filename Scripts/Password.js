...
async  function userDetail()
{
 ...
 function checkPassword(varempcd,varoldpwd) {
    return new Promise(function (resolve, reject) {
        $.ajax({
            type: "POST",
            url: "WebService.asmx/checkPassword",
...
var checkPasswordAuthentication =function(value){
...
async function btnChange_click()
{
    ...
    try 
    {
        ...
        var passmessage=checkPasswordAuthentication(txtChaPas.val());
        if (passmessage!=null) {
            ErrMsg =passmessage;
            throw ErrMsg;
        }
        ...
        if (usercd.length >0) 
        {
            ...
            var result = parseInt(res.responseObject.outres);
            if(result != 1) 
            {
            ...
            else 
            {
                ...
                objprpempsec.userpass = txtChaPas.val().trim();
                var result = await UpdatePassword(objprpempsec.emp_cd, objprpempsec.userpass)
                if (result.response == -1) {
                    ErrMsg = result.responseMsg;
                    throw ErrMsg; 
                }
                $("[id*=lblMessage]").text(result.responseMsg).css('color', '#008000'); // Green color
                toastr.success(result.responseMsg, {timeOut: 500});
                $("[id*=btnChange]").prop("disabled",true);
                setTimeout(window.location.href = "Account/Login.aspx", 10000);
            }
        }
    } 
