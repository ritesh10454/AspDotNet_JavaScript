...
$(function () {
    ChangeBreadCrumb("Action", "HOD Approval", "Action");
...
//         table.order([ 0, "desc" ] , [ 1, "desc" ] ).draw();
        ...
        $('#InboxTable').on('click', 'tbody tr #btnSelect',async function () {
            ...
            var encTag= await encryptQueryString("4");
...
function GetJRHODApprovalInbox(empcd) {
    return new Promise(function (resolve, reject) {
        $.ajax({
            type: "POST",
            url: "WebService.asmx/GetJRHODApprovalInbox",
            data: "{'empcd' : '" + empcd + "'}",
...
async function BindGrid() {
    ...
    try 
    {
        ...
        if(usercd.length > 0)
        {
            var res= await GetJRHODApprovalInbox(usercd.toString().trim());
            if (res.response ==-1) 
            {
                ShowEmptyTable();
