...
{
    var dt = new Date()
    var curHrs= dt.getHours();
    var curMin =dt.getMinutes();
    if (curHrs <=9) {curHrs= "0" + curHrs;}
    if (curMin <=9) {curMin= "0" + curMin;}
    var newTime = String.format("{0}:{1}",curHrs,curMin);
    return newTime;
}
...
//                $("[id*=txtdateofjoining]").val(ChangeDateFormat(objprpemployeedetail.dt_join));
                ...
                else if(tag==3)
                {
                    ...
                    $("[id*=btnaction]").hide();            
                    $("[id*=btnaction]").removeAttr("disabled");
                    var hrAuth = await JR_checkJRAuth(usercd.toString().trim());

                    if(hrAuth.response==1)  //       usercd.toString().trim()=="P17036")
                    {
                        $("[id*=btnaction]").show();
...
//                    if (curTm >="13:00") {
//                        var errm ="HOD can not approve JR after 06:00 PM";
//                        $("[id*=lblMessage]").text(errm).css('color', '#ff0000'); // Red color
//                        toastr.error(errm, {timeOut: 200});
//                        $(".loader").fadeOut();                       
//                        setTimeout(window.location.href = "HODApproval.aspx", 1000); 
//                        return;
//                    }
            ...
            if(jrId!=0)
            {
                ...
                if (tag==3 && finalID=="")  //Effective Date column is visible in case of HR authorization
                {
                    objprpjrhdr.revision_date=Convert_DateInStringFormat(new Date());
                }
			    $("[id*=txtjobdesc]").summernote('code',objprpjrhdr.jr_detail);
...
//                    }
                        objJRHDR.RoleId = 5; 
                        objJRHDR.finalapp_id = ucd.toString().trim(); 
                        objJRHDR.revision_date="1900-01-01";  
                        objJRHDR.btnaction="Forward";
    ...
    else
    {
        ...
        if (btnSave.text().trim() == "Save") 
        {
            ...
            if ($("[id*=drpEmployee] option:Selected").val() != "" )
            {
                ...
                var dupemp= await checkDuplicate(lblempcd.val());
                if(dupemp.response==-1)
                {
                    ErrMsg= dupemp.responseMsg;
                    throw ErrMsg;
                }
        ...
        else if (btnSave.text().trim() == "Update") 
        {
            ...
            if ($("[id*=drpEmployee] option:Selected").val() != "" )
            {
                ...
                else if(res1.CheckID==-2)
                {
                    ErrMsg="This Employee JR is already Exist";
                    throw ErrMsg;
                }
...
function checkDuplicate(empcd) {
    return new Promise(function (resolve, reject) {
        $.ajax({
            type: "POST",
            url: "WebService.asmx/checkDuplicate",
            data: "{'empcd' : '" + empcd + "'}",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            async: false,
...
function JR_checkJRAuth(usercd) {
    return new Promise(function (resolve, reject) {
        $.ajax({
            type: "POST",
            url: "WebService.asmx/checkJRAuth",
            data: "{'empcd' : '" + usercd + "'}",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            async: false,
