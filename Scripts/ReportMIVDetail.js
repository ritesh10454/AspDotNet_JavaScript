...
$(document).ready(function () {
    ChangeBreadCrumb("Report", "JR Detail", "Report");
...
function DateFormat(dt) {
    ...
    var year =Math.abs(d[2].substr(0,4));
    if (month < 10) {   month = '0' + month; }
    if (date < 10) {   date = '0' + date; }
...
function DateFormat2(dt) {
    var formatted_date = "";
    var d=null;var month=null;var year=null;var date=null;
    if (dt == null || dt == "") return formatted_date;
    if (isDate(dt)) {
        d =  new Date(dt);
        date = d.getDate();
...
async function btnPrint_Click(ctl)
{
    ...
    try 
    {
      $(".loader").fadeIn();
        ...
        var myData = await GetJRReportPrint(lblempcd,jrId);
        if (myData.response == -1) {
            alert(myData.responseMsg);
        }
       var reportData= myData.responseObject;
       var  _doj =  DateFormat2(reportData.doj);   
        ...
        tableHeading += '<tr><td colspan="4"><table width="100%"><tr><td style="text-align:left;">Ref: HRM/SOP/013</td><td style="text-align:right;">Doc # : HRM/F/065</td></tr></table></td></tr>';
//        tableHeading += '<tr><td colspan="3" style="text-align: left; font-family: "Times New Roman";font-size:12px; font-weight: bold;">Ref: HRM/SOP/013</td><td colspan="4" style="text-align: right; font-family: "Times New Roman";font-size:12px; font-weight: bold;">Doc # : HRM/F/065</td></tr>';
        tableHeading += '</table>';
        tableHeading += '</div>';
        /////-----------------------End----------------------//
...
//        tableHeading += String.format('<table cellpadding="0" cellspacing="0" width="100%" border="true"><tr><td style="text-align: Left;font-family: "Times New Roman";font-size:16px;">Name</td><td style="white-space: initial;">{0}</td><td style="text-align: Left;font-family: "Times New Roman";font-size:16px;">Designation / Grade</td><td style="white-space: initial;">{1}  / {2}</td></tr><tr><td style="text-align: Left;font-family: "Times New Roman";font-size:16px;">E. Code</td><td style="white-space: initial;">{3}</td><td style="text-align: Left;font-family: "Times New Roman";font-size:16px;">Date of Joining</td><td style="white-space: initial;">{4}</td></tr><tr><td style="text-align: Left;font-family: "Times New Roman";font-size:16px;">Department</td><td colspan="3" style="white-space: initial;">{5}</td></tr></table>',toTitleCase(reportData.emp_nm),toTitleCase(reportData.desig_nm),reportData.catg_nm,reportData.emp_cd,_doj,toTitleCase(reportData.dept_nm.split('-')[0]));
        ...
        tableHeading += '<table cellpadding="0" cellspacing="0" width="100%" border="true">';
        tableHeading += '<tr><td>Prepared By:</td><td>Acknowledged By:</td><td>Verified By HOD:</td><td>Approved by HRM:</td></tr>';
        ...
        tableHeading += '</tr>';
        tableHeading += '</table>';
        tableHeading += '<div style="text-align: Left;">Rev. # 04 (Effective date: 15.04.2023)</div>';
        tableHeading += '</div>';
        /////----------------------End-----------------------//
...
//            par =5;
            ...
            row["para_2"] = "JR Detail Report View - All Employee, All Department and JR Detail Type wiz";
            objPrpUsrLogDtl.push(row);

        var deptcd = drpdep.val();
        res= await GetJRReport(deptcd, fromDate, ToDate);
        if (res.response == -1) 
		{
            ErrMsg= res.responseMsg;
            throw ErrMsg;
...
//		var HTML_Width = $(".printfont").width();
//		var HTML_Height = $(".printfont").height();
...
//		html2canvas($(".printfont")[0],{allowTaint:true}).then(function(canvas) {
...
function createDataTable(myData)
{
  return new Promise(function (resolve, reject) 
  {
        var newRows = "";
        var tot=0;
        var tableHeading='<table id="JRDetailTable"  border="0" cellpadding="0" cellspacing="0"  class ="table table-striped" style="width:100%;"><thead><tr><th class="employeename">Employee Name</th><th class="employeecode">Employee Code</th><th class="revisionno">Revision Number</th><th class="effectivedate">Effective Date</th><th class="id">ID</th><th class="print">Print</th></tr></thead><tbody>';
        newRows += tableHeading;
        for (var i = 0; i < myData.length; i++) 
        {
            
             newRows +=     '<tr><td class="employeename">' + myData[i].emp_nm + 
                            '</td><td class="employeecode">' + myData[i].emp_cd + 
                            '</td><td class="revisionno">' + myData[i].revision_no +  
                            '</td><td class="effectivedate">' + myData[i].revision_date + 
                            '</td><td class="id">' + myData[i].Id + 
...
//        if (tot > 0) {
//            $('#JRDetailTable tfoot th:eq(8)').text(tot.toFixed(2));
//        }
//        else 
//        {
//            $('#JRDetailTable tfoot').hide();
//        }
        $("[id*=lblMessage]").text("");
            resolve("OK");
    });
...
function GetJRReportPrint(varempcd,jrId) {
    return new Promise(function (resolve, reject) {
        $.ajax({
            type: "POST",
            url: "WebService.asmx/GetJRReportPrint",
            data: "{'varempcd' : '" + varempcd + "','jrID' : " + jrId + " }",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            async: false,
            timeout: 5000,
