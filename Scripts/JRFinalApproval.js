...
$(function () {
    ChangeBreadCrumb("Action", "Final HR Approval", "Action");
...
//         table.order([ 0, "desc" ] , [ 1, "desc" ] ).draw();
        ...
        $('#InboxTable').on('click', 'tbody tr #btnSelect',async function () {
            ...
            var encTag= await encryptQueryString("3");
            var curTm = getCurrentTime();
            if (curTm >="18:00") 
            {
                var errm ="HR can not approve JR after 06:00 PM";
                toastr.error(errm, {timeOut: 1000});                     
                return;
            }
            else
            {
...
function GetJRFinalApprovalInbox(empcd) {
    return new Promise(function (resolve, reject) {
        $.ajax({
            type: "POST",
            url: "WebService.asmx/GetJRFinalApprovalInbox",
            data: "{'empcd' : '" + empcd + "'}",
...
async function BindGrid() {
    ...
    try 
    {
        ...
        if(usercd.length > 0)
        {
            var res= await GetJRFinalApprovalInbox(usercd.toString().trim());
            if (res.response ==-1) 
            {
                ShowEmptyTable();
